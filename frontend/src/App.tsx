...
function App() {
  ...
  const ticketAbi = useMemo(
    () => [
      ...
      // verifier
      'function verifyTicket(uint256,uint256,address) view returns (bool)'
    ],
    [],
  )
  ...
  // --- Verify ticket UI state ---
  const [verifyTokenId, setVerifyTokenId] = useState<string>('')
  const [verifyEventId, setVerifyEventId] = useState<string>('')
  const [verifyHolder, setVerifyHolder] = useState<string>('')
  const [verifyResult, setVerifyResult] = useState<boolean | null>(null)
  const [verifying, setVerifying] = useState(false)

  const handleVerifyTicket = useCallback(async () => {
    if (!baseTicketContract) {
      setStatusMessage({ type: 'error', text: 'Contract not loaded.' })
      return
    }

    const tokenIdNum = Number(verifyTokenId)
    const eventIdNum = Number(verifyEventId)
    if (!Number.isFinite(tokenIdNum) || !Number.isFinite(eventIdNum) || !ethers.utils.isAddress(verifyHolder)) {
      setStatusMessage({ type: 'error', text: 'Invalid tokenId, eventId, or holder address.' })
      return
    }
    ...
    try {
      ...
      setStatusMessage({ type: 'info', text: 'Checking ticket on-chain…' })
      const result: boolean = await baseTicketContract.verifyTicket(tokenIdNum, eventIdNum, verifyHolder)
      setVerifyResult(result)
      setStatusMessage({ type: 'success', text: result ? 'Ticket is valid for entry.' : 'Ticket is NOT valid.' })
    } catch (err) {
      setStatusMessage({ type: 'error', text: `Verification failed: ${(err as Error)?.message ?? 'Unknown error'}` })
    } finally {
      setVerifying(false)
    }
  }, [baseTicketContract, verifyTokenId, verifyEventId, verifyHolder])
                    ...
                    <div style={{ marginTop: '18px', display: 'flex', gap: '12px' }}>
                      ...
                      <div style={{ marginTop: 12, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <button
                          ...
                          disabled={!baseTicketContract || verifying}
                          style={{ padding: '8px 12px', borderRadius: 8, background: 'rgba(13,223,194,0.12)', color: '#0ddfc2' }}
                        >
                      ...
                      {verifyResult !== null && (
                        <p style={{ marginTop: 8, color: verifyResult ? '#0ddfc2' : '#ff72f9' }}>
                          {verifyResult ? '✅ Valid ticket for entry' : '❌ Invalid ticket or not active / wrong holder'}
                        </p>
                      )}
                    </div>
                  </article>
